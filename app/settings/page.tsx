  ...
  return (
    <div className="flex flex-col flex-1 bg-gray-50">
      <div className="bg-white border-b border-gray-100 px-4 pt-5 pb-4">
        ...
        <p className="text-sm text-gray-400 mt-0.5">Metas diarias, calculadora y mis alimentos</p>
      </div>

      <div className="flex-1 overflow-y-auto px-4 pt-5 pb-32 flex flex-col gap-4">
        {loading ? <p className="text-gray-400 text-sm text-center py-12">Cargando…</p> : (
          <>
            {/* ── METAS ─────────────────────────────────────────────── */}
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-1">Metas diarias</p>
            ...
            {MACRO_FIELDS.map(({ key, label, unit, color }) => (
              <div key={key} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-gray-400 ml-auto">{unit} / día</span>
                </div>
                <div className="flex items-center gap-3">
                  <input type="number" inputMode="numeric" value={goals[key] || ""}
                    ...
                    className={`flex-1 border rounded-xl px-4 py-3 text-2xl font-bold focus:outline-none focus:ring-2 ${RING_COLOR[color]}`} />
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
              <p className="text-xs font-medium text-gray-400 mb-2">Distribución calórica estimada</p>
            ...
            {/* ── CALCULADORA TDEE ──────────────────────────────────── */}
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-1 mt-2">Calcular metas</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-gray-700">Calculadora TDEE</span>
                </div>
                <svg className={`w-4 h-4 text-gray-400 transition-transform ${showTDEE ? "rotate-180" : ""}`}
              ...
              {showTDEE && (
                <div className="border-t border-gray-100 px-4 pb-4 flex flex-col gap-3">
                  {/* Género */}
                  <div className="grid grid-cols-2 gap-2 mt-3">
                    {(["male", "female"] as const).map((g) => (
                      <button key={g} onClick={() => setProfile((p) => ({ ...p, gender: g }))}
                        className={`py-2.5 rounded-xl text-sm font-medium transition-colors ${
                          profile.gender === g ? "bg-emerald-500 text-white" : "bg-gray-100 text-gray-600"}`}>
                        {g === "male" ? "♂ Hombre" : "♀ Mujer"}
                      </button>
                    ))}
                  </div>
                      ...
                      <div key={key}>
                        ...
                        <div className="relative">
                          <input type="number" inputMode="decimal" placeholder={placeholder}
                            ...
                            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-emerald-300" />
                  ...
                  {/* Actividad */}
                  <div>
                    <label className="text-xs text-gray-400 block mb-1">Actividad</label>
                    ...
                    {(Object.entries(GOAL_LABELS) as [Profile["goal"], string][]).map(([k, v]) => (
                      <button key={k} onClick={() => setProfile((p) => ({ ...p, goal: k }))}
                        className={`py-2.5 rounded-xl text-xs font-medium transition-colors text-center ${
                          profile.goal === k ? "bg-emerald-500 text-white" : "bg-gray-100 text-gray-600"}`}>
                        {v}
                      </button>
                    ))}
                  </div>
                  ...
                  {tdeeResult && (
                    <div className="bg-emerald-50 rounded-2xl p-3 flex flex-col gap-2">
                      ...
                      <div className="grid grid-cols-4 gap-2 text-center">
                        <div><p className="text-base font-bold text-emerald-600">{tdeeResult.calories}</p><p className="text-xs text-gray-400">kcal</p></div>
                        <div><p className="text-base font-bold text-blue-500">{tdeeResult.protein}g</p><p className="text-xs text-gray-400">prot.</p></div>
                        <div><p className="text-base font-bold text-amber-500">{tdeeResult.carbs}g</p><p className="text-xs text-gray-400">carbs</p></div>
                        <div><p className="text-base font-bold text-violet-500">{tdeeResult.fat}g</p><p className="text-xs text-gray-400">grasa</p></div>
                      </div>
            ...
            {/* ── MIS ALIMENTOS ─────────────────────────────────────── */}
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-1 mt-2">Mis alimentos</p>

            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              {customFoods.length > 0 && customFoods.map((food) => (
                <div key={food.id} className="flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-0">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-800 truncate">{food.nombre}</p>
                    <p className="text-xs text-gray-400">{food.cal} kcal · {food.p}g P · {food.c}g C · {food.f}g G — por 100g</p>
                  </div>
                  <button onClick={() => deleteCustomFood(food.id)} className="p-2 text-gray-300 hover:text-red-400 active:text-red-500 transition-colors flex-shrink-0">
              ...
              {/* Formulario añadir */}
              {showAddFood ? (
                <div className="px-4 py-4 flex flex-col gap-3 border-t border-gray-50">
                  <p className="text-sm font-semibold text-gray-700">Nuevo alimento</p>
                  ...
                  <p className="text-xs text-gray-400 -mb-1">Por cada 100g:</p>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setShowAddFood(false); setNewFood({ nombre: "", cal: "", p: "", c: "", f: "" }); }}
                      className="flex-1 border border-gray-200 text-gray-600 font-medium rounded-xl py-2.5 text-sm active:bg-gray-50">
                      Cancelar
                    </button>
                    <button onClick={handleAddFood} disabled={!newFood.nombre.trim() || !Number(newFood.cal)}
                      className="flex-[2] bg-emerald-500 disabled:bg-gray-200 disabled:text-gray-400 text-white font-semibold rounded-xl py-2.5 text-sm active:bg-emerald-600">
                      Guardar
                    </button>
                  </div>
                </div>
                ...
                <button onClick={() => setShowAddFood(true)}
                  className="w-full flex items-center gap-2 px-4 py-3.5 text-sm font-medium text-emerald-600 active:bg-gray-50 transition-colors">
      ...
      <div className="fixed bottom-16 left-0 right-0 px-4 pb-2 max-w-[430px] mx-auto">
        <button onClick={handleSave} disabled={saving || loading}
          className={`w-full font-semibold rounded-2xl py-4 transition-all flex items-center justify-center gap-2 shadow-lg ${
            saved ? "bg-emerald-500 text-white" : "bg-gray-800 text-white hover:bg-gray-700 disabled:bg-gray-200 disabled:text-gray-400"}`}>
          {saving ? (
